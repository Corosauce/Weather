...
public class EntTornado extends Entity implements WindHandler
{
    ...
    {
        if (blockID != 0 && WeatherMod.shouldGrabBlock(blockID))
        {
            return true;
        }
    ...
    {
        return this.worldObj.rayTraceBlocks(Vec3.createVectorHelper(this.posX, this.posY + (double)this.getEyeHeight(), this.posZ), Vec3.createVectorHelper((double)x, (double)y, (double)z)) == null;
    }
    ...
    public boolean tryPlaySound(String[] sound, int arrIndex, Entity source, float vol)
    {
        ...
        if (this.soundTimer[arrIndex] <= System.currentTimeMillis())
        {
            ...
            this.soundID[arrIndex] = WeatherMod.playMovingSound(new StringBuilder().append("tornado." + sound[snd_rand[arrIndex]]).toString(), (float)soundTarget.posX, (float)soundTarget.posY, (float)soundTarget.posZ, vol, 1.0F);
    ...
    public void onUpdate()
    {
        ...
        //}
        //}
        int lockTime = (int)((float)WeatherMod.stormMan.stormStartTime * 0.4F);

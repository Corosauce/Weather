...
import weather.entities.EntityBuoyant;
...
public class WeatherManager
{
    ...
    public void updateWater(World world)
    {
        ...
        for (int ii = 0; ii < worldRef.loadedEntityList.size(); ii++)
        {
            ...
            if (entity1.worldObj.isRemote && entity1 instanceof EntityPlayer && entity1.onGround && !entity1.isInWater()) {
            	int oceanLevel = 63;
    ...
    public boolean isBoyant(Entity ent)
    {
        ...
        {
            if (WeatherUtil.getIsJumping((EntityLivingBase)ent))
            {
                return true;
            }
        }
    ...
    public void applyBoyancy(Entity ent)
    {
        ...
        if (bdp.isWater)
        {
            ...
            if ((ent.posY + ent.getEyeHeight() + offsetFix) - 1.0F <= top)
            {
                ...
                //theres a glitch where entities will be falling from sky to their death when you move into area, assumed horrible wave issue, this is a patch
                //second possibility that the 'is in wave' data is bad and its constantly pushing it up till i move in, if this fix doesnt work, that might be the case
                if (ent.motionY > 0.15) {
                	ent.motionY = 0.15;
                }
    ...
    public void tick(Side side, World world)
    {
        ...
        boolean windOn = ((ConfigWind.Wind_active && world.provider.dimensionId == WeatherUtil.mainDimID) || (ConfigWind.TropicraftRealm_Wind_active && world.provider.dimensionId == WeatherUtil.tropiDimID));
